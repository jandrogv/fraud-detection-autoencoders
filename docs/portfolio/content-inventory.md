...
| Standalone Python source (`*.py`) | No `.py` file was present during the updated audit; `Trabajo_Final.ipynb` is the available executable source. | A missing source file prevents confirmation of the intended public implementation basis. | `NEEDS OWNER CONFIRMATION` | Provide the intended Python file or confirm that the notebook is the sole source of truth. | `Owner Confirmation` Confirm that the notebook is the sole source of truth and the only code used. There is another code `Pruebas autoencoder 0.ipynb` with all the tests that i made before the cretion of the final `Trabajo_Final.ipynb`
