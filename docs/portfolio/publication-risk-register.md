...
|---|---|---|---|---|---|---|
| R1 | Dataset origin and redistribution rights unknown | `creditcard.csv`; `Trabajo_Final.ipynb`, cells 4-5 | No licence, source URL, permission, or data card exists in the repository. The CSV is ignored by Git. | High: public redistribution cannot be assumed. | Obtain owner confirmation of source and licence; otherwise use synthetic data and a generic data-access note. | `NEEDS OWNER CONFIRMATION` |
...
| R10 | Narrative/artifact inconsistency | `Trabajo_Final.ipynb`, cells 86, 95, 110, 119; `Modelos/*_hist.npy` | Some stated training limits do not exactly match recorded history lengths. | Medium: weakens the credibility of unqualified training claims. | Treat all values as reported; reconcile or omit epoch claims after rerunning. | `NEEDS OWNER CONFIRMATION` |
| R11 | Language and academic-format mismatch | `Trabajo_Final.ipynb`; `Trabajo_Final.pdf` | The technical narrative is primarily Spanish and the PDF is an academic-length report. | Medium: copying it would not meet the English recruiter-facing objective. | Produce an English executive summary, concise README, and recreated figures. | `SUMMARISE` |
